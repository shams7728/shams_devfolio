 ...
 * 
 * Main page for managing roles with CRUD operations and drag-and-drop reordering
...
import { DraggableRoleList } from './draggable-role-list';
...
export default function AdminRolesPage() {
  ...
  const handleReorder = async (reorderedRoles: Role[]) => {
    const items = reorderedRoles.map((role, index) => ({
      id: role.id,
      display_order: index,
    }));
  ...
  const handleTogglePublish = async (role: Role) => {
    const response = await fetch(`/api/roles/${role.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ is_published: !role.is_published }),
    });
  ...
  // Handle configure background
  const handleConfigureBackground = (role: Role) => {
    window.location.href = `/admin/roles/${role.id}/background`;
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="mt-4 text-zinc-600 dark:text-zinc-400">Loading roles...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-zinc-600 dark:text-zinc-400">
            Create and organize your professional roles
          </p>
        </div>
          ...
          {roles.length === 0 ? (
            ...
            <>
              ...
              <DraggableRoleList
                ...
                onEdit={handleEdit}
                onDelete={(role) => setDeleteConfirm(role)}
                onTogglePublish={handleTogglePublish}
                onConfigureBackground={handleConfigureBackground}
              />
            </>
          )}
        </>
